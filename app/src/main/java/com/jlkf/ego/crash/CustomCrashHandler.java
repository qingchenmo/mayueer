...
public class CustomCrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		...
		//提示用户程序即将退出
		showToast(mContext, AppUtil.getResString(mContext, R.string.tip_crash_label));
		try {
			Thread.sleep(2000);
...
//        System.exit(1);
	...
	private HashMap<String, String> obtainSimpleInfo(Context context){
		...
		map.put("time", AppUtil.getNowTimeString());
	...
	private String obtainExceptionInfo(Throwable throwable) {
		...
		AppLog.Loge(TAG, mStringWriter.toString());
		//此方法会把传入的错误信息发回服务器
//		UMGameAgent.reportError(mContext,mStringWriter.toString());
	...
	private String savaInfoToSD(Context context, Throwable ex){
		...
		if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
			File dir = new File(SDCARD_ROOT + File.separator + "domainStar/crash" + File.separator);
			if(! dir.exists()){
				dir.mkdir();
			}
	...
	private String paserTime(long milliseconds) {
		...
		SimpleDateFormat format = new SimpleDateFormat(AppUtil.getVersionName(mContext)+".yyyyMMddHHmmss");
